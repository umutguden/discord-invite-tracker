...
mongoose.connect('mongodb+srv://heimdall:lf4dvex1FsiAocnk@cluster0.fczxxkr.mongodb.net/?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect linki
  ...
  if (isMemberFake) {
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) inviteChannel.send(`${member} katıldı! **Davet eden**: ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** davet <:cross:1016314311548874823>)`).catch(err => {});
        });
      } else {
        inviterData.fake++
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
        if (!inviterData) {
          ...
          newInviter.save().then(x => {
            if (inviteChannel) inviteChannel.send(`${member} katıldı! **Davet eden**: ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** davet <:check:1016314242548371486>)`).catch(err => {});
          });
        } else {
          inviterData.regular++;
          inviterData.save().then(x => {
...
const arr = ["387675598044135436","647070332171059200","647780762170425355"]
if(!arr.includes(message.author.id)) return message.reply({content: `Bonus davet eklemek yalnızca Hanedan'a özeldir.`});
  ...
  if (command === "üyeler" || command === "members") {
    ...
    Database.find({guildID: message.guild.id, inviterID: uye.id}).sort([["descending"]]).exec(async (err, pageArray) => {
      ...
      if (!pageArray.length) {
        ...
        let reactions = ["◀", "<:cross:1016314311548874823>", "▶"];
        ...
        const back = msg.createReactionCollector((reaction, user) => reaction.emoji.name == "◀" && user.id == message.author.id,
              { time: 20000 }),
            x = msg.createReactionCollector((reaction, user) => reaction.emoji.name == "<:cross:1016314311548874823>" && user.id == message.author.id, 
              { time: 20000 }),
